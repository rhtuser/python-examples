...
#
#   (venv) $ flask --app ./code/ch11/guestbook --debug run
#
import json
import os
import re
...
from guest import Guest, GuestEncoder

guestfile = "code/ch11/guests.json"
app = Flask(__name__)
    ...
    def get_customers():
        with open(guestfile, "r") as gb:
            for customer in gb.readlines():
                yield json.loads(customer, object_hook=Guest.from_dict)

    for customer in get_customers():
        if customer.firstname == user.firstname and customer.middlename == user.middlename and customer.lastname == user.lastname:
...
def store_user_data(user: Guest):
    ...
    with open(guestfile + ".tmp", "w") as wb:
        for customer in get_customers():
            if customer.firstname == user.firstname and customer.middlename == user.middlename and customer.lastname == user.lastname:
                ...
                updated = True
                print(json.dumps(user, cls=GuestEncoder), file=wb)
            else:
                print(json.dumps(customer, cls=GuestEncoder), file=wb)
    ...
    with open(guestfile, "a") as ab:
        ...
        print(json.dumps(user, cls=GuestEncoder), file=ab)
